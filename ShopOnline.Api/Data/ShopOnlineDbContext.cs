...
namespace ShopOnline.Api.Data
{
    ...
    .UsingEntity<TagProduct>(
        right => right.HasOne(e => e.Product).WithMany(),
        left => left.HasOne(e => e.Tag).WithMany().HasForeignKey(e => e.TagId),
        join => join.ToTable("TagProduct")
    );
            modelBuilder.Entity<TagProduct>().HasData(
                new TagProduct() { TagId = 1, ProductId = 1 });

            modelBuilder.Entity<TagProduct>().HasData(
                new TagProduct() { TagId = 2, ProductId = 1 });
        }
